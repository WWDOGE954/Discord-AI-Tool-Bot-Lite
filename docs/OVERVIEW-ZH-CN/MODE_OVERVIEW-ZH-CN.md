...
## 为什么 MCP-like 模式仍共用 router 的功能逻辑

> 简答：为了轻量化与核心概念的聚焦

在本项目中，`mcp_like_mod.py` 主要用来展示「工具注册」与「工具调用」的概念。

为了让项目保持轻量，`mcp_like` 模式并没有重新写一整套 AI、usage、memory 的处理逻辑，而是尽量共用 router 模式中已经整理好的功能。

这样做的好处是：

* 避免重复实现相同功能
* 让两种模式的使用者功能保持一致
* 让 `mcp_like_mod.py` 专注展示工具注册架构
* 让项目维持小型、容易阅读与容易比较

也就是说，这个版本的 `mcp_like` 比较像是加在工具层上的「注册展示层」，而不是完整取代所有底层功能的独立 MCP 系统。

它没有实现：

* MCP client/server 通讯
* MCP protocol message format
* 外部工具 discovery
* 完整 schema 验证
* 真正由 AI 自主选择工具

目前它只是用轻量方式展示「工具注册表」的概念。
...
这样设计的原因是，本项目主要目标是保持简单、可读、容易展示，而不是实现完整的 MCP 系统。
