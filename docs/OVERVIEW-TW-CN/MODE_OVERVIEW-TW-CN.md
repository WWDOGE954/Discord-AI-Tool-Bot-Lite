...
Discord response
```
## 為什麼 MCP-like 模式仍共用 router 的功能邏輯

> 簡答: 為了輕量化與核心概念的聚焦

在本專案中，`mcp_like_mod.py` 主要用來展示「工具註冊」與「工具呼叫」的概念。

為了讓專案保持輕量，`mcp_like` 模式並沒有重新寫一整套 AI、usage、memory 的處理邏輯，而是盡量共用 router 模式中已經整理好的功能。

這樣做的好處是：

* 避免重複實作相同功能
* 讓兩種模式的使用者功能保持一致
* 讓 `mcp_like_mod.py` 專注展示工具註冊架構
* 讓專案維持小型、容易閱讀與容易比較

也就是說，這個版本的 `mcp_like` 比較像是加在工具層上的「註冊展示層」，而不是完整取代所有底層功能的獨立 MCP 系統。
...
這樣設計的原因是，本專案主要目標是保持簡單、可讀、容易展示，而不是實作完整的 MCP 系統。
